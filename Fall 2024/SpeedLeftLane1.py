def reward_function(params):
    # Calculate center variance and retrieve relevant variables
    ...
    # Define racing lanes as sets for O(1) lookups
    ...
    # Define speed zones as sets for O(1) lookups
    ...
    reward += 10 if all_wheels_on_track else -10

    # Reward for staying on the correct lane
    if closest_waypoint in left_lane and is_left_of_center:
        reward += 10
    elif closest_waypoint in right_lane and not is_left_of_center:
        reward += 10
    elif closest_waypoint in center_lane and center_variance < 0.4:
        reward += 10
    else:
        reward -= 10
    ...
    if closest_waypoint in fast:
        reward += 10 if speed > 1.5 else -10
    elif closest_waypoint in moderate:
        reward += 10 if 1 <= speed <= 1.5 else -10
    elif closest_waypoint in slow:
        reward += 10 if speed <= 1 else -10
